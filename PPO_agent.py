...
from vec_env_handler import ParallelEnvManager
...
def orthogonal_init(model, gain='relu'):
    for layer in model:
...
class PPO():
    ...
    def __init__(
        ...
        if models: self.model, self.value_net = models
            ...
            self.value_net = default_model(input_space=observation_space, output_space=1)
