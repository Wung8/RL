...
class SoccerEnv:
    ...
    def get_rewards(self, player):
        player = self.player_idxs[player]
    ...
    def get_inputs(self, player):
        ...
        goals = [(50,225),(50,375),(850,225),(850,375)]
        if team: goals = [(850,225),(850,375),(50,225),(50,375)]
        for goal in goals:
    ...
    def reset(self):
        ...
        for player in self.players:
            player.reset()
        random.shuffle(self.player_idxs)
    ...
    def step(self, actions, display=False):
        ...
        for i in range(4):
            for j in range(4):
                self.players[self.player_idxs[j]].step(actions[j])
        ...
        if self.ball_body.position[0] < 50:
            rewards = [[-1,-1,1,1][self.player_idxs[i]] for i in range(4)]
            return [self.get_inputs(i) for i in range(4)], np.array(rewards)*self.reward_scaling, True
