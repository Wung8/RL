...
# keeping pendulum up
def r1(x, theta): return ((-math.sin(theta)+1)/2)#**2# - 2*max(0,abs(x)-.7)
...
def nextFrame(usr,display=False):
    ...
    #if abs(theta%(2*pi)-1.5*pi) > pi/2: return -1, -0, [0,0,0], True
    ...
    # ( x, angle, vel, t ), r
    return ((pos[0]-400)/200 * 2, theta, vel, step/500), rfun((pos[0]-400)/200, theta), [1,1,1], False
...
def resetEnv():
    ...
    pos = (400,400)
    step = 0
    return ((pos[0]-400)/200 * 2, theta, vel, step/500), [1,1,1]
