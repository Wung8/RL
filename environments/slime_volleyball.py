    ...
    def getInputs(self, left=True):
        if left:
            return (*self.scale(self.shift(self.slime_left.pos,0),1/400),
                    ...
                    *self.scale(self.shift(self.ball.pos,0),1/800),
                    *self.scale(self.ball.vel,1/60))
        else:
            return (*self.scale(self.shift(self.flip_pos(self.slime_right.pos),0),1/400),
                    *self.scale(self.shift(self.flip_pos(self.slime_left.pos),-400),1/400), 
                    *self.scale(self.shift(self.flip_pos(self.ball.pos),0),1/800),
                    *self.scale(self.flip_vel(self.ball.vel),1/60))

    def nextFrame(self, action1, action2, display=False, is_skip=False):
        self.timestep += 1
        ...
        if is_skip == False:
            for i in range(self.skip_frames):
                result = self.nextFrame(action1, action2, display=False, is_skip=True)
                if result == -1:
        ...
        return (self.getInputs(), self.getInputs(left=False)), (.0001 * ball_side, .0001 * -ball_side), False
...
if __name__ == '__main__':
    ...
    while True:
        ...
        update_usr1()
        update_usr2()
        obs, r, done = env.nextFrame(usr1, usr2, display=True)
        if done:
            print(r)
            time.sleep(0.5)
            env.resetEnv()
